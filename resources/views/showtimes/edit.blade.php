        ...
        <div class="form-group">
            ...
            <select class="form-control" id="theaterId" name="theater_id">
                @foreach ($theaters as $theater)
                    <option value="{{ $theater->id }}" @if ($theater->id == $showtime->theater_id) selected @endif>{{ $theater->theater_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select class="form-control" id="screenId" name="screen_id">
                @foreach ($screens as $screen)
                    <option value="{{ $screen->id }}" @if ($screen->id == $showtime->screen_id) selected @endif>{{ $screen->screen_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="showtime">Showtime:</label>
            <input type="datetime-local" class="form-control" id="showtime" name="showtime" value="{{ $showtime->showtime->format('Y-m-d\TH:i:s') }}">
        </div>
        <div class="form-group">
            <label for="price">Price:</label>
            <input type="number" step="0.01" class="form-control" id="price" name="price" value="{{ $showtime->price }}">
        </div>
